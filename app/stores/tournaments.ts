...
import type { Tournament, NewTournament, TournamentStats, TournamentFilters } from '~/types'
...
export const useTournamentsStore = defineStore('tournaments', () => {
  ...
  const filteredTournaments = computed(() => {
    return tournaments.value.filter(tournament => {
      ...
      // Date range filter
      if (!isDateInRange(tournament.date, f.dateRange)) return false

      // Type filter
      if (f.type !== 'all' && tournament.type !== f.type) return false

      // Currency filter
      if (f.currency !== 'all' && tournament.currency !== f.currency) return false

      // Buy-in range filter
      if (f.buyInMin !== undefined && tournament.buyIn < f.buyInMin) return false
      if (f.buyInMax !== undefined && tournament.buyIn > f.buyInMax) return false
