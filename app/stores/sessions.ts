...
import type { CashSession, NewCashSession, SessionStats, SessionFilters } from '~/types'
...
export const useSessionsStore = defineStore('sessions', () => {
  ...
  const filteredSessions = computed(() => {
    return sessions.value.filter(session => {
      ...
      // Date range filter
      if (!isDateInRange(session.date, f.dateRange)) return false

      // Type filter
      if (f.type !== 'all' && session.type !== f.type) return false

      // Game filter
      if (f.game !== 'all' && session.game !== f.game) return false

      // Currency filter
      if (f.currency !== 'all' && session.currency !== f.currency) return false

      // Stakes filter
      if (f.stakes.length > 0 && !f.stakes.includes(session.stake)) return false
      ...
      // Profit range filter
      if (f.minProfit !== undefined && session.result < f.minProfit) return false
      if (f.maxProfit !== undefined && session.result > f.maxProfit) return false
