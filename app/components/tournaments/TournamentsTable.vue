      ...
      <table class="min-w-full divide-y divide-border dark:divide-border-dark">
        <thead class="bg-surface-secondary dark:bg-surface-dark-tertiary">
          <tr>
            ...
            <th class="px-2 min-[1440px]:px-4 py-3 text-left text-xs font-medium text-foreground-muted dark:text-foreground-dark-muted uppercase tracking-wider">
              Date
            </th>
            <th class="px-2 min-[1440px]:px-4 py-3 text-left text-xs font-medium text-foreground-muted dark:text-foreground-dark-muted uppercase tracking-wider">
              Name
            ...
            <th class="px-2 min-[1440px]:px-4 py-3 text-right text-xs font-medium text-foreground-muted dark:text-foreground-dark-muted uppercase tracking-wider">
              Buy-in
            </th>
            <th class="px-2 min-[1440px]:px-4 py-3 text-left text-xs font-medium text-foreground-muted dark:text-foreground-dark-muted uppercase tracking-wider">
              Finish
            ...
            <!-- Pinned right so the row actions stay reachable when the table scrolls -->
            <th class="sticky right-0 z-20 bg-surface-secondary dark:bg-surface-dark-tertiary px-2 min-[1440px]:px-4 py-3 text-right text-xs font-medium text-foreground-muted dark:text-foreground-dark-muted uppercase tracking-wider">
              Actions
            </th>
          </tr>
        </thead>
        <tbody class="bg-surface dark:bg-surface-dark-secondary divide-y divide-border-subtle dark:divide-border-dark-subtle">
          <template v-for="tournament in tournaments" :key="tournament.id">
            <tr
              class="row"
              :class="[
                tournament.status === 'in_progress' ? 'row-in-progress' : '',
                hasMultipleSites(tournament) ? 'cursor-pointer' : '',
              ]"
            ...
            >
              ...
              <td class="px-2 min-[1440px]:px-4 py-3 whitespace-nowrap text-sm text-foreground dark:text-foreground-dark">
                {{ formatDate(tournament.date) }}
              </td>
              <td class="px-2 min-[1440px]:px-4 py-3 text-sm text-foreground dark:text-foreground-dark">
                <!-- Names run long; cap them so the money columns keep their space -->
                <div class="flex items-center gap-2 max-w-[11rem] min-[1440px]:max-w-[18rem] min-[1700px]:max-w-[24rem]">
                  <span class="truncate" :title="tournament.name">
                    {{ tournament.name }}
                  </span>
                  <span
                    v-if="tournament.source === 'desktop'"
                    class="shrink-0 badge-neutral badge-pill text-2xs"
                    title="Added from desktop"
                  >
                    Desktop
                  </span>
                </div>
              </td>
              <td class="px-2 min-[1440px]:px-4 py-3 whitespace-nowrap text-sm">
                <span
                  class="badge"
              ...
              <td class="px-2 min-[1440px]:px-4 py-3 text-sm text-foreground-muted dark:text-foreground-dark-muted">
                <span v-if="getSiteCount(tournament) > 1" class="whitespace-nowrap">
                  {{ getSiteCount(tournament) }} {{ tournament.type === 'live' ? 'venues' : 'sites' }}
                </span>
                <span v-else class="block max-w-[9rem] truncate" :title="getSiteLabel(tournament)">
                  {{ getSiteLabel(tournament) }}
                </span>
              </td>
              <td class="px-2 min-[1440px]:px-4 py-3 whitespace-nowrap text-sm text-right font-mono text-foreground-muted dark:text-foreground-dark-muted">
              ...
              <td class="px-2 min-[1440px]:px-4 py-3 whitespace-nowrap text-sm">
                ...
                <template v-else>
                  <span
                    v-if="tournament.finishPosition"
                    class="text-foreground dark:text-foreground-dark"
                    :title="getFinishTitle(tournament)"
                  >
                    {{ formatPosition(tournament.finishPosition) }}
                    <span v-if="tournament.fieldSize" class="hidden min-[1440px]:inline text-foreground-muted dark:text-foreground-dark-muted">
              ...
              <td
                class="px-2 min-[1440px]:px-4 py-3 whitespace-nowrap text-sm text-right font-semibold font-mono"
              ...
              <td class="sticky right-0 z-10 px-2 min-[1440px]:px-4 py-3 whitespace-nowrap text-right text-sm">
                <div class="flex gap-1 min-[1440px]:gap-2 justify-end">
                  <NuxtLink
...
// The field size is dropped on narrower screens, so keep it available on hover.
function getFinishTitle(tournament: Tournament): string {
  if (!tournament.finishPosition) {
    return '';
  }
  return tournament.fieldSize
    ? `${tournament.finishPosition} of ${tournament.fieldSize}`
    : formatPosition(tournament.finishPosition);
}

function getSiteLabel(tournament: Tournament): string {
  return (tournament.type === 'live' ? tournament.venue : tournament.site) || '';
}
...
<style scoped>
/*
 * The Actions column is pinned to the right edge, so the row tint has to sit on
 * the cells rather than the row: a translucent tint on a pinned cell lets the
 * columns scrolling underneath show through it. Same colors as the utilities
 * they replace, composited onto the surface instead of layered over it.
 */
.row > td {
  background-color: oklch(var(--s-surface));
  transition: background-color var(--dur-micro) var(--ease-out);
}

.row:hover > td {
  background-color: color-mix(in oklab, oklch(var(--s-raised)) 50%, oklch(var(--s-surface)));
}

.row-in-progress > td {
  background-color: oklch(var(--wa-100));
}

.row-in-progress:hover > td {
  background-color: color-mix(in oklab, oklch(var(--wa-200)) 50%, oklch(var(--s-surface)));
}

:root.dark .row-in-progress > td {
  background-color: color-mix(in oklab, oklch(var(--wa-900)) 30%, oklch(var(--s-surface)));
}

:root.dark .row-in-progress:hover > td {
  background-color: color-mix(in oklab, oklch(var(--wa-900)) 40%, oklch(var(--s-surface)));
}
</style>
